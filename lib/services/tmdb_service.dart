...
class TmdbService {
  ...
  Future<Map<String, dynamic>> getMovieDetails(
    ...
    final uri = Uri.parse('$_baseUrl/movie/$id').replace(
      queryParameters: {
        'api_key': apiKey,
        'language': language,
        'append_to_response':
            'credits,images', // Include actors, crew, and images (logos, etc.)
      },
    );
  ...
  Future<List<Map<String, dynamic>>> searchTvShow(
    String query, {
    int? year,
    String language = 'it-IT',
  }) async {
  ...
  Future<Map<String, dynamic>> getTvShowDetails(
    ...
    final uri = Uri.parse('$_baseUrl/tv/$id').replace(
      queryParameters: {
        'api_key': apiKey,
        'language': language,
