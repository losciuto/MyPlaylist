...
class _FilterDialogState extends State<FilterDialog> {
  ...
  Future<void> _loadData() async {
    try {
      final genres = await db.AppDatabase.instance.getValuesWithCounts(
        'genres',
      );
      final years = await db.AppDatabase.instance.getValuesWithCounts('year');
      final actors = await db.AppDatabase.instance.getValuesWithCounts(
        'actors',
      );
      final directors = await db.AppDatabase.instance.getValuesWithCounts(
        'directors',
      );
      ...
      final sortedGenres = Map.fromEntries(
        genres.entries.toList()..sort((a, b) => a.key.compareTo(b.key)),
      );
      final sortedYears = Map.fromEntries(
        years.entries.toList()..sort((a, b) => b.key.compareTo(a.key)),
      ); // Years desc
      final sortedActors = Map.fromEntries(
        actors.entries.toList()..sort((a, b) => a.key.compareTo(b.key)),
      );
      final sortedDirectors = Map.fromEntries(
        directors.entries.toList()..sort((a, b) => a.key.compareTo(b.key)),
      );

      // Filter sagas to show only those with more than 1 movie
      final filteredSagas = Map.fromEntries(
        sagas.entries.where((e) => e.value > 1).toList()
            ...
            Container(
              ...
              child: ListView(
                children: [
                  _buildSidebarItem(
                    0,
                    AppLocalizations.of(context)!.tabGeneral,
                    Icons.settings,
                  ),
                  _buildSidebarItem(
                    1,
                    AppLocalizations.of(context)!.tabGenres,
                    Icons.category,
                  ),
                  _buildSidebarItem(
                    2,
                    AppLocalizations.of(context)!.tabYears,
                    Icons.calendar_today,
                  ),
                  _buildSidebarItem(
                    3,
                    AppLocalizations.of(context)!.tabDirectors,
                    Icons.person,
                  ),
                  _buildSidebarItem(
                    4,
                    AppLocalizations.of(context)!.tabActors,
                    Icons.group,
                  ),
                  _buildSidebarItem(
                    5,
                    AppLocalizations.of(context)!.tabSagas,
                    Icons.library_books,
                  ),
      ...
      case 0:
        return Column(
          ...
          children: [
            ...
            Text(
              AppLocalizations.of(context)!.maxVideos,
            ...
            Text(
              AppLocalizations.of(context)!.minRating,
              style: const TextStyle(color: Colors.white70),
            ),
            Slider(
              value: _minRating,
            ...
            Center(
              child: Text(
                AppLocalizations.of(context)!.filterByRating(_minRating),
                style: const TextStyle(color: Colors.white70),
              ),
            ),
          ],
  ...
  Widget _buildSummary() {
    ...
    int exclusions =
        ...
        _excludedDirectors.length +
        _excludedSagas.length;

    if (inclusions == 0 && exclusions == 0) {
      return Padding(
        padding: const EdgeInsets.all(10.0),
        child: Text(
          AppLocalizations.of(context)!.noActiveFilters,
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          if (inclusions > 0)
            Text(
              AppLocalizations.of(context)!.included(inclusions),
              style: const TextStyle(color: Color(0xFF4CAF50), fontSize: 11),
            ),
          if (exclusions > 0)
            Text(
              AppLocalizations.of(context)!.excluded(exclusions),
