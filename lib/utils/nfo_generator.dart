  ...
  static String generateFromVideo(model.Video video) {
    ...
    builder.element(
      ...
      nest: () {
        ...
        builder.element(
          'rating',
          attributes: {'name': 'app', 'max': '10', 'default': 'true'},
          nest: () {
        ...
        // Directors
        final directorNames = video.directors
            .split(',')
            .map((e) => e.trim())
            .where((e) => e.isNotEmpty)
            .toList();
        final directorThumbs = video.directorThumbs
            .split('|')
            .map((e) => e.trim())
            .toList();
        ...
        // Actors
        final actorNames = video.actors
            .split(',')
            .map((e) => e.trim())
            .where((e) => e.isNotEmpty)
            .toList();
        final actorThumbs = video.actorThumbs
            .split('|')
            .map((e) => e.trim())
            .toList();
        ...
        if (video.posterPath.isNotEmpty) {
          builder.element(
            'thumb',
            attributes: {'aspect': 'poster'},
            nest: video.posterPath,
          );
        }
  ...
  static String generateMovieNfo(Map<String, dynamic> tmdbData) {
    ...
    builder.element(
      ...
      nest: () {
        ...
        builder.element(
          'userrating',
          nest: tmdbData['vote_average']?.toString() ?? '0.0',
        );
        // Kodi standard rating block
        builder.element(
          'rating',
          attributes: {'name': 'tmdb', 'max': '10', 'default': 'true'},
          nest: () {
            builder.element(
              'value',
              nest: tmdbData['vote_average']?.toString() ?? '0.0',
            ...
            builder.element(
              'votes',
              nest: tmdbData['vote_count']?.toString() ?? '0',
            );
          },
        );
  ...
  static String generateTvShowNfo(Map<String, dynamic> tmdbData) {
    ...
    builder.element(
      ...
      nest: () {
        ...
        final firstAirDate =
            tmdbData['first_air_date'] as String?; // YYYY-MM-DD
        ...
        if (tmdbData['episode_run_time'] != null &&
            (tmdbData['episode_run_time'] as List).isNotEmpty) {
          builder.element(
            'runtime',
            nest: tmdbData['episode_run_time'][0]?.toString() ?? '',
          );
        }

        if (tmdbData['networks'] != null &&
            (tmdbData['networks'] as List).isNotEmpty) {
          builder.element(
            'studio',
            nest: tmdbData['networks'][0]['name'] ?? '',
          );
        }
