...
### Fixed
- **Rating Parser**: Fixed a critical bug in the NFO parser that ignored the correct rating if the `<userrating>` field was present but set to zero (common in files generated by TinyMediaManager/Kodi). Correct priority is now given to values in the `<ratings>` block.
